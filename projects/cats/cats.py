...
def about(topic):
    ...
    # BEGIN PROBLEM 2
    def check(s):
        slist=split(remove_punctuation(lower(s)))
        for word in topic:
            for sword in slist:
                if word==sword:
                    return True
...
def autocorrect(user_word, valid_words, diff_function, limit):
    ...
    for word in valid_words:
        if user_word==word:
            return user_word 
        if diff_function(user_word,word,limit) < minn:
            minn=diff_function(user_word,word,limit)
            answer[minn]=word
    if minn>limit:
        return user_word
    else:
...
def time_per_word(times_per_player, words):
    ...
    for index in range(len(times_per_player)):
        timelist.append([times_per_player[index][i]-times_per_player[index][i-1] for i in range(1,len(times_per_player[index]))])
    return game(words,timelist)
    # END PROBLEM 9
...
def fastest_words(game):
    ...
    players = len(times)
    fastestlist = [[] for i in range(players)]
    for i,word in enumerate(words):
        wordtime = [times[player][i] for player in range(players)]
        fastindex = min(range(players), key=lambda x: wordtime[x])
        fastestlist[fastindex].append(word)
    return fastestlist
