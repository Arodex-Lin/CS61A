...
def reverse_recursive(lst):
    ...
    def help(list):
        ...
        a=[]
        a.append(list[-1])
        #a.extend(help(list[0:len(list)-1]))
        a +=help(list[0:len(list)-1])
        return a
    return help(lst)
...
def closer_city(lat, lon, city_a, city_b):
    ...
    return get_name(city_a) if distance(tem,city_a)<=distance(tem,city_b) else get_name(city_b)
...
def add_chars(w1, w2):
    ...
    def help(s1,s2):
        if len(s1)==0:
            return s2 
        if s1[0]==s2[0]:
            return ''+help(s1[1:],s2[1:])
        else:
            return s2[0]+help(s1,s2[1:])
    return help(w1,w2)
